...
const CopyButton = ({ url }: { url: string }) => {
  ...
  return (
    <button onClick={handleCopy} title="Copy link" className="text-zinc-500 hover:text-blue-400 transition-colors">
      {copied ? (
...
export default function Home() {
  ...
  // Compute difficulty analytics locally
  useEffect(() => {
  ...
  useEffect(() => { setIsGuest(!localStorage.getItem("authToken")); }, []);

  useEffect(() => {
    fetchData();
    if (!isGuest) fetchAnalytics();
  }, [activeView, isGuest]);
  ...
  const handleAiAutoFill = async () => {
    ...
    const token = localStorage.getItem("authToken");
    if (!token) { alert("AI Scan requires an account. Sign in to use this feature!"); return; }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("authToken");
    if (!token) {
      const newResource = { id: Date.now(), title, url, category, difficulty, createdAt: new Date().toISOString() };
    ...
    try {
      const response = await authFetch("/api/resources", {
        method: "POST",
        body: JSON.stringify({ title, url, category, difficulty: difficulty.trim() !== "" ? difficulty : null }),
      });
  ...
  const handleSaveEdit = async () => {
    ...
    if (!token) {
      const current = JSON.parse(localStorage.getItem("guestResources") || "[]");
      const updated = current.map((r: any) => r.id === editingResource.id ? { ...r, title: editTitle, category: editCategory, note: editNote } : r);
      localStorage.setItem("guestResources", JSON.stringify(updated));
      setResources(updated);
  ...
  // Bulk archive — single API call
  const handleBulkArchive = async () => {
    const token = localStorage.getItem("authToken");
    if (!token) {
      // Guest mode: archive each locally
      const current = JSON.parse(localStorage.getItem("guestResources") || "[]");
      const updated = current.map((r: any) => selectedIds.includes(r.id) ? { ...r, archived: true } : r);
      localStorage.setItem("guestResources", JSON.stringify(updated));
      setResources(updated.filter((r: any) => !r.archived));
    } else {
      try {
        const response = await authFetch("/api/resources/bulk", {
          method: "DELETE",
  ...
  const filteredResources = (() => {
    let result = selectedCategory ? resources.filter((r: any) => r.category === selectedCategory) : resources;
    if (searchQuery.trim()) {
        ...
        <CardHeader className="p-5">
          <div className="flex justify-between items-center mb-5">
            ...
            <div className="flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
              ...
              <a href={resource.url} target="_blank" rel="noopener noreferrer" className="text-zinc-500 hover:text-blue-400 transition-colors ml-1" onClick={(e) => e.stopPropagation()}>
  ...
  // Domain grouping renderer
  const renderResourceGrid = () => {
    const domainsWithMany = Object.entries(domainCounts).filter(([, count]) => count >= 3).map(([domain]) => domain);
    ...
    return (
      <div className="space-y-8">
        {Object.entries(grouped).map(([domain, domainResources]) => {
          ...
          return (
            <div key={domain}>
              <button onClick={() => toggleDomain(domain)} className="flex items-center gap-2 mb-3 group w-full text-left">
    ...
    <main className="min-h-screen bg-black text-[#a1a1aa] font-sans antialiased selection:bg-blue-500 selection:text-white pb-24 relative overflow-hidden">
      ...
      <nav className="relative z-10 flex justify-between items-center px-6 md:px-12 py-8 max-w-7xl mx-auto">
        ...
        <div className="flex gap-8 text-sm font-medium text-zinc-500 items-center">
          ...
          {!isGuest && (
            <button onClick={handleLogout} className="cursor-pointer text-zinc-600 hover:text-red-400 transition-colors ml-2 flex items-center gap-1.5">
            ...
            <div className="h-52">
              {analyticsData.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={analyticsData} dataKey="value" nameKey="name" cx="50%" cy="45%" outerRadius={62} innerRadius={28} paddingAngle={3}>
                      {analyticsData.map((entry, index) => (
                        <Cell key={`cell-${index}`} fill={DIFFICULTY_COLORS[index % DIFFICULTY_COLORS.length]} />
                      ))}
                    </Pie>
                    <Tooltip contentStyle={{ backgroundColor: '#18181b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', fontSize: '11px' }} formatter={(value: any, name: any) => [`${value} resource${value !== 1 ? 's' : ''}`, name]} />
                    <Legend wrapperStyle={{ fontSize: '10px', paddingTop: '8px' }} formatter={(value) => <span style={{ color: '#a1a1aa' }}>{value}</span>} />
                  </PieChart>
                </ResponsiveContainer>
              )}
            </div>
          ...
          {activeView === "tracker" && (
            <div className="mb-16">
              <form onSubmit={handleSubmit} noValidate className="flex flex-col xl:flex-row gap-4 items-end">
                  ...
                  <div className="flex gap-2">
                    <Input type="text" placeholder="https://..." value={url}
                      onChange={(e) => {
                        ...
                        if (e.target.value === "") { setUrlError(null); }
                        else if (!isValidUrl(e.target.value)) { setUrlError("Please enter a valid URL (e.g. https://example.com)"); }
                    ...
                    />
                    <Button type="button" onClick={handleAiAutoFill}
                ...
                <Button type="submit" disabled={Boolean(urlError || !url || resources.some((r: any) => r.url.toLowerCase() === url.toLowerCase()))}
                  className="bg-blue-600/90 text-white hover:bg-blue-600 rounded-xl h-12 px-8 font-semibold shadow-md shrink-0 w-full xl:w-auto disabled:opacity-50">
                  Deploy
                </Button>
              </form>
            </div>
              ...
              <>
                <button onClick={() => setSelectedCategory(null)}
                  className={`px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-all border ${
                    selectedCategory === null ? "bg-blue-600/20 text-blue-400 border-blue-500/40" : "text-zinc-500 border-white/10 hover:text-zinc-300 hover:border-white/20"
                  }`}>
                  All
                </button>
                {allCategories.map((cat: string) => (
                  <button key={cat} onClick={() => setSelectedCategory(cat === selectedCategory ? null : cat)}
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-all border ${
                      selectedCategory === cat ? "bg-blue-600/20 text-blue-400 border-blue-500/40" : "text-zinc-500 border-white/10 hover:text-zinc-300 hover:border-white/20"
                    }`}>
                    {cat}
                  </button>
                ))}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 mt-8">
          <div className="bg-blue-600/10 border border-blue-500/50 p-4 rounded-xl mb-8">
            <p className="text-blue-200 text-sm">
              ...
              <button onClick={() => router.push('/login')} className="ml-2 underline font-bold hover:text-white transition-colors">Sign in</button>
              {" or "}
              <button onClick={() => router.push('/register')} className="underline font-bold hover:text-white transition-colors">create an account</button>
          ...
          <div className="bg-zinc-900 border border-white/10 rounded-2xl p-8 w-full max-w-md shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <textarea value={editNote} onChange={(e) => setEditNote(e.target.value)} placeholder="Why did you save this? What's it for?" rows={3}
      ...
      <div className={`fixed bottom-8 left-1/2 -translate-x-1/2 transition-all duration-500 z-50 ${
        ...
        <div className="flex items-center gap-6 bg-zinc-900/80 backdrop-blur-md border border-white/10 px-6 py-3 rounded-full shadow-2xl">
          ...
          <button onClick={() => setSelectedIds([])} className="text-zinc-400 hover:text-white text-sm transition-colors">
            Cancel
          </button>

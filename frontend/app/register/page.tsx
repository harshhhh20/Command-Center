...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError(data.message || "Registration failed. Username may already be taken.");
        return;
      }
      ...
      // 2. Migrate any guest data to the new account
      const guestData = localStorage.getItem("guestResources");
      if (guestData && JSON.parse(guestData).length > 0) {
        setMigrationStatus("Syncing your guest data to your new account...");
        const count = await migrateGuestData(token);
        setMigrationStatus(`Synced ${count} resource${count !== 1 ? "s" : ""} to your account!`);
  ...
  return (
    <main className="min-h-screen bg-black text-[#a1a1aa] font-sans antialiased flex items-center justify-center relative overflow-hidden">
      ...
      {/* Ambient glow — consistent with Command Center & Login */}
      ...
      <div className="relative z-10 w-full max-w-sm px-6">
        ...
        >
          ...
          >
            {isLoading ? "Creating Account..." : "Sign Up & Sync Data"}
          </Button>
        </form>
        ...
        <p className="text-center text-zinc-600 text-[10px] tracking-widest uppercase mt-6">
          Command Center · New Recruit Registration
        </p>
      </div>
    </main>
  );
